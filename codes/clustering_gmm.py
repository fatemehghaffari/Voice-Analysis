...
comps = [2, 5, 10, 50, 100, 200, 400]
for c in comps:
    gmm = GaussianMixture(n_components = c, covariance_type = 'diag').fit(flatten_features)
    labels = gmm.predict(flatten_features)
    cluster_avg_bd = []
    mses = 0
    for i in range(c):
...
plt.legend()
plt.show()
ard3 = plt.plot(comps, ard_g, label = 'Adjusted rand index')
ami3 = plt.plot(comps, ami_g, label = 'Adjusted mutual information')
h3 = plt.plot(comps, h_g, label = 'Homogeneity')
c3 = plt.plot(comps, c_g, label = 'Completeness')
v3 = plt.plot(comps, v_g, label = 'V-measure')
fm3 = plt.plot(comps, fm_g, label = 'Fawlkes Mallows score')
plt.legend()
plt.show()